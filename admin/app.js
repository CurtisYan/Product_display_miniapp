...
function openItem(categoryIndex,itemIndex){editingItem={categoryIndex,itemIndex};const category=catalog.categories[categoryIndex],item=itemIndex<0?{id:'P'+Date.now(),name:'新展示项',subtitle:'',description:'',category:category.name,images:[]}:itemsFor(category)[itemIndex],form=$('#editor form');for(const key of ['id','name','subtitle','description'])form.elements[key].value=item[key]||'';renderImageFields((item.images||[]).map(imageUrl));$('#delete').style.visibility=itemIndex<0?'hidden':'visible';$('#editor').showModal()}
$('#editor').addEventListener('close',()=>{if($('#editor').returnValue!=='save'||!editingItem)return;const {categoryIndex,itemIndex}=editingItem,category=catalog.categories[categoryIndex],form=$('#editor form'),item={category:category.name};for(const key of ['id','name','subtitle','description'])item[key]=form.elements[key].value.trim();item.images=getImageValues().filter(Boolean).map(imageUrl);if(itemIndex<0)catalog.products.push(item);else{const old=itemsFor(category)[itemIndex],position=catalog.products.indexOf(old);catalog.products[position]=item}editingItem=null;render()})
