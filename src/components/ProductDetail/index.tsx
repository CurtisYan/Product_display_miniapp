    ...
    >
      <ScrollView scrollY className='detail-scroll' onScroll={event => { scrollTop.current = event.detail.scrollTop }}>
        <View className='detail-inner'>
          ...
          <Text className='eyebrow'>PRODUCT SAMPLE · {product.id}</Text>
          <View className='detail-title'>{product.name}</View>
          <Swiper className='detail-swiper' circular indicatorDots>
            {product.images.map(src => <SwiperItem key={src}><Image src={src} mode='aspectFit' showMenuByLongpress /></SwiperItem>)}
          </Swiper>
          {product.images.length > 1 && <Text className='detail-image-tip'>左右滑动查看 {product.images.length} 张图片</Text>}
          ...
          <View className='detail-actions'>
            ...
            <Button openType='share' data-product-id={product.id} onClick={() => saveShareRecord(product)}>分享 ↗</Button>
          </View>
        </View>
      </ScrollView>
    </View>
